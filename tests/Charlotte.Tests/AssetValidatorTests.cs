...
public class AssetValidatorTests
{
    ...
    {
        Assert.DoesNotContain(AssetValidator.CheckClip(AnimationId.Battle,Enumerable.Range(0,13).Select(i=>new FrameSpec($"b/{i}.png",195)).ToArray()),x=>x.Code=="total-duration");
        Assert.DoesNotContain(AssetValidator.CheckClip(AnimationId.Victory,Enumerable.Range(0,10).Select(i=>new FrameSpec($"v/{i}.png",210)).ToArray()),x=>x.Code=="total-duration");
        Assert.Contains(AssetValidator.CheckClip(AnimationId.Battle,Enumerable.Range(0,13).Select(i=>new FrameSpec($"b/{i}.png",250)).ToArray()),x=>x.Code=="total-duration");
