...
- `scripts/test-publish-layout.ps1` 自动刷新上述含空格路径副本，并从系统临时目录再次通过 12 动作、94 帧资源加载检查。
- 同一脚本只在 `artifacts/release asset replacement` 隔离副本中用另一张有效透明帧替换 Idle 首帧；替换前后 SHA-256 不同、资源检查仍通过，标准发布目录哈希保持不变。
...
- 本轮发布布局回归不重新启动 GUI；“从含空格路径且不依赖 SDK 启动、正常退出后无残留进程”沿用本分支此前的真实进程证据，正式帧替换后的视觉效果仍须随正式美术资产复验。
