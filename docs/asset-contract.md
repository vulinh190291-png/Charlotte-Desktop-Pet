...
当前清单处于 `formal` 阶段：`assets/character/formal-v1/` 保存 Idle、Rest、Sleep、Drag、Battle、Victory 共 69 张正式透明帧。Walk 与三组 Click 动画已退出当前清单，旧 `assets/character/generated/` 占位目录已删除。正式素材的原文件名、SHA-256、输出路径和画布变换记录在 `assets/character/formal-v1/source-map.json`。

- 逻辑画布：480×600；默认显示：240×300 DIP。
- 运行时可见 foot anchor：(240,540)；所有人物帧保持同一画布和脚部基准。
...
- 每个动作通过 `overlayEffects` 声明是否使用独立、完全穿透的特效窗口。正式 Sleep、Drag、Battle、Victory 已将气泡、玫瑰、花瓣或星光画入帧内，因此关闭程序叠加。
- 资源路径必须位于 assets 根目录内；根目录以下的 Junction、符号链接或其他重解析点均不作为合法素材路径。

`config/animations.json` 的 `behavior` 节点保存 3 分钟 Rest、10 分钟 Sleep、Idle 静止/微动 70/30 比例，并明确关闭自动 Walk。旧清单兼容字段仍会被解析与校验，但当前清单没有 Walk/Click clip。左键点击仅作为交互事件唤醒角色并重置闲置计时，不会切换到缺失动画，也不会产生无动画的窗口滑行。
