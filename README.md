...
Windows 10/11 x64 桌面宠物 MVP。当前版本使用 69 张经过统一画布处理的正式 Q 版透明动画帧，资产替换规则见 `docs/asset-contract.md`。
...
- 左键单击角色会唤醒休息/睡眠并重置闲置计时；拖动可把角色停留在任意桌面位置。
...
- 无交互 3 分钟后进入休息、10 分钟后进入睡眠；当前版本不启用自动散步。
